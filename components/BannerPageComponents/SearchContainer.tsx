...
const SearchContainer = () => {
  ...
  return (
    <div className="w-full flex gap-[30px] flex-col 1000px:flex-row">
      ...
      <button
      name="Search banners"
      className={`${searchBanners.length === 0 ? 'cursor-not-allowed bg-[#71A7EF]' : 'cursor-pointer bg-[#196fe1]'} text-white px-[16px] h-[40px] rounded-full text-[14px] font-bold`}>
        Search banners
      </button>
      ...
      onClick={()=>{ dispatch(enableAddBannerComponentShow()) }}
      >
        Add banner
      </button>
      </div>
